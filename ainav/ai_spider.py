...
# 正式: scrapy runspider ai_spider.py -o ainav.csv
# 测试: scrapy runspider ai_spider.py -o ainav_test.csv

class AISpider(scrapy.Spider):
    name = "ai_spider"
    allowed_domains = ["www.ainav.cn"]
    start_urls = []

    for i in range(1600, 0, -1):
        start_urls.append("https://www.ainav.cn/sites/%s.html" % i)
    ...
    # 测试使用
    # start_urls = ['https://www.ainav.cn/sites/1173.html', 'https://www.ainav.cn/sites/1564.html']
    ...
    def parse(self, response):
        ...
        try:
            ...
            yield {
                ...
                'used_model': None,
                'product_display_link': self.get_product_display_link(response.xpath(xpath_str_map.get("product_display_link")).extract_first()),
    ...
    def get_product_display_link(self, url):
        headers = {
            'Referer': 'https://www.ainav.cn/sites/1.html',
        }

        req_response = requests.get(url, headers=headers)

        # 转换到 Scrapy Response
        scrapy_response = TextResponse(url=req_response.url, status=req_response.status_code,
                                       headers=dict(req_response.headers), body=req_response.text, encoding='utf-8')

        product_link = scrapy_response.xpath("/html/body/div[2]/div/div[2]/div[2]/span").xpath('string()').get().strip()
        return product_link
